...
const Customercategory = ({ navigation }) => {
    ...
    const hasCategoryPermissions = permissions['Customer Type'] || {};
    ...
    const Updatecategory = async (id) => {
        ...
        setLoading(true);
        const url = `${Constant.URL}${Constant.OtherURL.update_customer_category}`;
        const response = await fetch(url, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
    ...
    const openModal = (category, x, y) => {
        ...
        setAddcat(category.category_name);
    ...
    const handleDelete = async (id) => {
        const url = `${Constant.URL}${Constant.OtherURL.delete_customer_category}`;
        const response = await fetch(url, {
            method: 'DELETE',
            headers: {
                'Content-Type': 'application/json',
    ...
    const confirmDelete = () => {
        Alert.alert(
            "Delete Category",
            `Are you sure you want to delete ${selectedCategory.category_name} customer type?`,
            [
                {
                    text: "Cancel",
                    style: "cancel"
                ...
                {
                    text: "Yes",
                    onPress: () => handleDelete(selectedCategory.customer_category_id)
                }
            ]
        );
    };
    ...
    return (
        <View style={{ flex: 1 }}>
            <Subheader headername='All Customer Type' />
                        ...
                        catlist.map((category, index) => (
                            <View key={index} style={{ flexDirection: 'row', justifyContent: 'space-between', borderRadius: 10, padding: 10, borderColor: '#E0E0E0', borderWidth: 1, backgroundColor: '#fff', marginTop: 10 }}>
                                <View style={{ flexDirection: 'row', width: '80%' }}>
                                    <Text style={{ fontFamily: 'Inter-Bold', fontSize: 14, color: '#173161' }}>{category.category_name}</Text>
                                </View>
            ...
            {hasCategoryPermissions.Add && (
                <View style={{ margin: 10, justifyContent: 'flex-end' }}>
                    <MyButton btnname="Create New Type" background="#173161" fontsize={18} textcolor="#FFF" runFunc={() => setCatModal(true)} />
                </View>
            )}
