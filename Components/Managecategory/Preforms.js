...
const Preforms = () => {
    ...
    const hasCategoryPermissions = permissions['Preform'] || {};

    const Addcategory = async () => {
        if (!addcat || addcat.trim() == '') {
            setError('Preform is required'); // Set error if validation fails
            return;
        }
        setLoading(true);
        const url = `${Constant.URL}${Constant.OtherURL.add_preform}`;
        const response = await fetch(url, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
        ...
        if (result.code == "200") {
            ...
            console.log('error while adding bag');
        }
        setLoading(false);
    };
    ...
    const Updatecategory = async (id) => {
        ...
        setLoading(true);
        const url = `${Constant.URL}${Constant.OtherURL.update_preform}`;
        const response = await fetch(url, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
    ...
    const handleDelete = async (id) => {
        const url = `${Constant.URL}${Constant.OtherURL.delete_preform}`;
        const response = await fetch(url, {
            method: 'DELETE',
            headers: {
                'Content-Type': 'application/json',
    ...
    const confirmDelete = () => {
        Alert.alert(
            "Delete Category",
            `Are you sure you want to delete ${selectedCategory.preform_type} bag ?`,
            [
                {
                    text: "Cancel",
                    style: "cancel"
    ...
    return (
        <View style={{ flex: 1 }}>
            <Subheader headername='All Preform Type' />
            <View style={{ margin: 10, flex: 1 }}>
                <ScrollView keyboardShouldPersistTaps='handled'>
                    {catlist.length == 0 ? (
                        <Text style={{ fontFamily: 'Inter-Bold', fontSize: 14, color: '#173161', alignSelf: 'center', marginTop: 100 }}>No Preform available</Text>
                    ) : (
                        catlist.map((item, index) => (
                            <View key={index} style={{ flexDirection: 'row', justifyContent: 'space-between', borderRadius: 10, padding: 10, borderColor: '#E0E0E0', borderWidth: 1, backgroundColor: '#fff', marginTop: 10 }}>
                                <View style={{ flexDirection: 'row', width: '80%' }}>
                                    <Text style={{ fontFamily: 'Inter-Bold', fontSize: 14, color: '#173161' }}>{item.preform_type}</Text>
                                </View>
            ...
            {hasCategoryPermissions.Add && (
                <View style={{ margin: 10, justifyContent: 'flex-end' }}>
                    <MyButton btnname="Create New Preform" background="#173161" fontsize={18} textcolor="#FFF" runFunc={() => setCatModal(true)} />
                </View>
            )}
                ...
                <TouchableOpacity onPress={() => { setCatModal(false); setAddcat(''); setError(''); }} style={{ flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: 'rgba(0,0,0,0.5)' }}>
                    <View onStartShouldSetResponder={(e) => e.stopPropagation()} style={{ width: '80%', backgroundColor: '#fff', paddingHorizontal: 20, paddingBottom: 20, paddingTop: 10, borderRadius: 10 }}>
                        <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 10 }}>
                            <Text style={{ fontSize: 16, fontFamily: 'Inter-Regular', color: '#000' }}>Preform
                        ...
                        <TextInput
                            placeholder="Enter Preform"
                            placeholderTextColor='gray'
                            value={addcat}
                            onChangeText={setAddcat}
                            ...
                            <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                                <TouchableOpacity onPress={() => {
                                    if (!selectedCategory) {
                                        ...
                                        Updatecategory(selectedCategory.id); setModalvisible(false); // Call Updatecategory if addcat is not empty
                                    }
