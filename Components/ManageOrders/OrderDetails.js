...
const OrderDetails = ({ navigation, route }) => {
                ...
                if (from == 'margin') {
                    ...
                    navigation.reset({
                        ...
                        routes: [
                            ...
                            { name: 'OrderList', params: { order_no, filterdate: formatteddate, filterstatus: valuestatus, filtertext: searchTerm } }    // Navigate to Userslist
                        ],
                    });
                }
    ...
    const handleDeletePayment = async (paymentId) => {
        try {
            const response = await fetch(
                `${Constant.URL}${Constant.OtherURL.delete_payment}`,
                {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
    ...
    const addpayment = async () => {
        ...
        try {
            ...
            const url = `${Constant.URL}${Constant.OtherURL.add_payment}`;
            const response = await fetch(url, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    const updatepayment = async () => {
        ...
        setLoading(true);
        try {
            const user_id = await AsyncStorage.getItem('admin_id');

            const paymentMethodLabel =
                paymentmethoditems.find(item => item.value == paymentmethodvalue)?.label
                || paymentmethodvalue;

            const response = await fetch(
                `${Constant.URL}${Constant.OtherURL.update_payment}`,
                {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                    ...
                    <TouchableOpacity onPress={() => {
                        if (from == 'margin') {
                            ...
                            navigation.reset({
                                ...
                                routes: [
                                    { name: 'Dashboard' },  // Keep HomeScreen in the stack
                                    { name: 'OrderList', params: { order_no, filterdate: formatteddate, filterstatus: valuestatus } }    // Navigate to Userslist
                                ],
                            });
                        }
                    }}>
