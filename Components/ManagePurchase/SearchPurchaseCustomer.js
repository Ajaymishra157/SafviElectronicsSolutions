...
const SearchPurchaseCustomer = ({ navigation }) => {
    ...
    const handleAddOrder = () => {
        ...
        if (permissions?.["Credit Permission"]?.["Over Limit"]) {
            ...
            navigation.replace('CreatePurchase', {
                companyname: selectedCompany.vendor_name,
                mobile_no: selectedCompany.vendor_mobile,
                customerid: selectedCompany.vendor_id,
    ...
    const navigatetoAddcustomer = () => {
        navigation.navigate('AddVendor', { mobile_no: searchTerm, from: 'SearchCustomer' });
        setIsModalVisible(false);
        setSearchTerm(null);
    };
    ...
    return (
        <View style={{ flex: 1 }}>
                    ...
                    <View style={{ gap: 10, marginTop: 10, marginHorizontal: 0 }}>
                        ...
                        >
                            <Text style={{ fontFamily: 'Inter-Regular', fontSize: 14, color: '#173161', textTransform: 'uppercase' }}>
                                {selectedCompany.vendor_name}{selectedCompany.area ? ` (${selectedCompany.area})` : ''}
                            </Text>
                            <Text style={{ fontFamily: 'Inter-Regular', fontSize: 12, color: '#173161', textTransform: 'uppercase' }}>
                                {selectedCompany.vendor_email}
                            </Text>
                            <Text style={{ fontFamily: 'Inter-Regular', fontSize: 12, color: '#173161', textTransform: 'uppercase' }}>
                                {selectedCompany.vendor_mobile}
                            </Text>
            ...
            {/* <View style={{ margin: 10, }}>
                {isOkButtonVisible ?
                    <MyButton btnname="Add Purchase" background={selectedCompany && isOkButtonPressed ? "#173161" : "#D3D3D3"} fontsize={18} textcolor={selectedCompany && isOkButtonPressed ? "#fff" : "#000"} runFunc={handleAddOrder} disabled={!selectedCompany || !isOkButtonPressed} />
                    : <MyButton btnname="Add Purchase" background={selectedCompany ? "#173161" : "#D3D3D3"} fontsize={18} textcolor={selectedCompany ? "#fff" : "#000"} runFunc={handleAddOrder} disabled={!selectedCompany} />
                }
            </View> */}
...
export default SearchPurchaseCustomer