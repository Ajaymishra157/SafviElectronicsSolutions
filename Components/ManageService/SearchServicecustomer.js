...
const SearchServicecustomer = ({ navigation }) => {
    ...
    const handleAddSevice = () => {
        ...
        if (permissions?.["Credit Permission"]?.["Over Limit"]) {
            ...
            navigation.navigate('OrderListService', {
                customerid: selectedCompany.c_id,
                customerName: selectedCompany.full_name
            });
    ...
    const navigatetoAddcustomer = () => {
        navigation.navigate('AddCustomer', { mobile_no: searchTerm, from: 'SearchServicecustomer' });
        setIsModalVisible(false);
        setSearchTerm(null);
    };
        ...
        <View style={{ flex: 1 }}>
            ...
            <View style={{ margin: 10, flex: 1 }}>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                    <TextInput placeholder='Search by Customer Name or Mobile' value={searchTerm} onChangeText={handleSearch} placeholderTextColor='gray' style={{ width: /^\d{10}$/.test(searchTerm) ? '90%' : '100%', fontFamily: 'Inter-Medium', fontSize: 14, color: '#173161', borderWidth: 1, borderColor: '#E0E0E0', backgroundColor: '#fff', borderRadius: 15, paddingHorizontal: 10 }} />
            ...
            {/* <View style={{ margin: 10, }}>
                {isOkButtonVisible ?
                    <MyButton btnname="Add Purchase" background={selectedCompany && isOkButtonPressed ? "#173161" : "#D3D3D3"} fontsize={18} textcolor={selectedCompany && isOkButtonPressed ? "#fff" : "#000"} runFunc={handleAddSevice} disabled={!selectedCompany || !isOkButtonPressed} />
                    : <MyButton btnname="Add Purchase" background={selectedCompany ? "#173161" : "#D3D3D3"} fontsize={18} textcolor={selectedCompany ? "#fff" : "#000"} runFunc={handleAddSevice} disabled={!selectedCompany} />
                }
            </View> */}
...
export default SearchServicecustomer