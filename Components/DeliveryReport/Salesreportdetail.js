...
const Salesreportdetail = ({ navigation, route }) => {
  const { startdate, enddate, product_id, product_name, totalqty, totalamt } = route.params || {};
  ...
  const fetchdata = async () => {
    try {
      setMainloading(true);
      const url = `${Constant.URL}${Constant.OtherURL.party_sales_report}`;
        ...
        {mainloading ? (
          ...
          <>
            ...
            <View style={{ borderRadius: 10, padding: 10, marginBottom: 10, borderColor: '#173161', borderWidth: 1, backgroundColor: '#fff' }}>
              <Text style={{ fontFamily: 'Inter-Regular', fontSize: 14, color: '#173161', marginBottom: 5, width: '90%', }}>{product_name}</Text>
