...
const DeliveryManReport = ({ navigation }) => {
    ...
    const fetchdeliverymanreport = async () => {
        ...
        const url = `${Constant.URL}${Constant.OtherURL.man_delivery_report}`;
    ...
    const totalDeliverySum = data.reduce((sum, item) => sum + parseFloat(item.delivered_order || 0), 0);
    const totalBoxSum = data.reduce((sum, item) => sum + parseInt(item.delivered_qty || 0), 0);

    return (
        <View style={{ flex: 1 }}>
            <Subheader headername="Delivery Man Report" />
                        ...
                        {data && data.length > 0 ? (
                            <View style={{ marginHorizontal: 5, borderRadius: 10, padding: 0, marginBottom: 5, borderColor: '#173161', borderWidth: 1, backgroundColor: '#fff' }}>
                                ...
                                {data.map((item, index) => (
                                    <View key={index} style={{ flexDirection: 'row', borderTopWidth: 1, borderColor: '#173161' }}>
                                        ...
                                        <View style={{ flexDirection: 'row', width: '45%', borderRightWidth: 1, borderColor: '#173161', paddingLeft: 5, alignItems: 'center' }}>
                                            ...
                                            <TouchableOpacity onPress={() => navigation.navigate('Deliverymanreportdetails', { startdate: startDate, enddate: endDate, userid: item.user_id, name: item.name, delivered_order: item.delivered_order, delivered_qty: item.delivered_qty })} style={{ width: '20%', alignItems: 'center', paddingVertical: 5 }}>
                                        ...
                                        <View style={{ width: '20%', borderRightWidth: 1, borderColor: '#173161', justifyContent: 'center', alignItems: 'center' }}>
                                            <Text style={{ color: '#173161', fontFamily: 'Inter-SemiBold', fontSize: 12, textTransform: 'uppercase' }}>{item.delivered_order}</Text>
                                        </View>
                                        <View style={{ width: '20%', justifyContent: 'center', alignItems: 'center' }}>
                                            <Text style={{ color: '#173161', fontFamily: 'Inter-SemiBold', fontSize: 12, textTransform: 'uppercase' }}>{item.delivered_qty}</Text>
                                        </View>
                                    </View>
                                ))}
                                ...
                                <View style={{ flexDirection: 'row', borderTopWidth: 1, borderColor: '#173161' }}>
                                    ...
                                    <View style={{ width: '20%', borderRightWidth: 1, borderColor: '#173161', justifyContent: 'center', alignItems: 'center' }}>
                                        <Text style={{ color: '#173161', fontFamily: 'Inter-SemiBold', fontSize: 12, textTransform: 'uppercase' }}>{totalDeliverySum}</Text>
                                    </View>
                                    <View style={{ width: '20%', justifyContent: 'center', alignItems: 'center' }}>
