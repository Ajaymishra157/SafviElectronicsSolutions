...
const VendorsList = ({ navigation }) => {
    ...
    const listvendors = async (text = searchTerm) => {
        ...
        const url = `${Constant.URL}${Constant.OtherURL.list_vendor}`;
        const response = await fetch(url, {
            method: 'POST',
            headers: {
    ...
    const handleDelete = async (id) => {
        const url = `${Constant.URL}${Constant.OtherURL.delete_vendor}`;
        const response = await fetch(url, {
            method: 'DELETE',
            headers: {
                'Content-Type': 'application/json',
    ...
    const confirmDelete = () => {
        Alert.alert(
            "Delete Vendor",
            `Are you sure you want to delete ${selecteduser} Vendor?`,
            [
                {
                    text: "Cancel",
                    style: "cancel"
    ...
    const openModal = (item, x, y) => {
        ...
        setSelectedUser(item.vendor_name);
        setselecteduserid(item.vendor_id);
    ...
    const handleEdit = () => {
        if (selectedcustomer) {
            navigation.navigate('AddVendor', { customerdata: selectedcustomer });
            setModalvisible(false);
        }
    };
    ...
    const handleaddcustomer = () => {
        if (selectedcustomer) {
            navigation.navigate('AddVendor', { customerdata: selectedcustomer, customer: 'Customer' });
            setModalvisible(false);
        }
    };
    ...
    const onRefresh = React.useCallback(() => {
        setRefreshing(true);
        setSearchTerm(null);
        listvendors().then(() => setRefreshing(false));
    }, []);
    ...
    return (
        <View style={{ flex: 1 }}>
            <View style={{ backgroundColor: '#173161', flexDirection: 'row', height: 50, alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                <Text style={{ color: '#FFF', fontFamily: 'Inter-Bold', fontSize: 16, marginLeft: 0 }}>All Vendors</Text>
            ...
            <View style={{ margin: 10 }}>
                <TextInput placeholder='Search by Vendor name or mobile' value={searchTerm}
                    onChangeText={listvendors} placeholderTextColor='gray' style={{ fontFamily: 'Inter-Medium', fontSize: 14, color: '#173161', borderWidth: 1, borderColor: '#E0E0E0', backgroundColor: '#fff', borderRadius: 15, paddingHorizontal: 10 }} />
            </View>
                                ...
                                <TouchableOpacity disabled={true} key={index} style={{ gap: 10, marginBottom: 10 }}>
                                    <View style={{ borderRadius: 10, padding: 10, borderColor: '#E0E0E0', borderWidth: 1, backgroundColor: '#fff', }}>
                                        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                                            <TouchableOpacity onPress={() => navigation.navigate('CustomerwiseOrders', { companyname: item.vendor_name, area: item.area, customerid: item.c_id })} style={{ width: '50%' }}>
                                                <Text style={{ fontFamily: 'Inter-Regular', fontSize: 14, color: '#173161', textTransform: 'uppercase' }}>{item.vendor_name}{item.vendor_email ? ` (${item.vendor_email})` : ''}</Text>
                                            </TouchableOpacity>
                                            <View style={{ flexDirection: 'row' }}>
                                                <TouchableOpacity onPress={() => Linking.openURL(`tel:${item.vendor_mobile}`)} style={{ padding: 8 }}>
                                                    <Image source={require('../../assets/phone-call.png')} style={{ height: 18, width: 18, tintColor: '#000' }} />
                                                </TouchableOpacity>
                                                <TouchableOpacity onPress={() =>
                                                    Linking.openURL(
                                                        `whatsapp://send?phone=${encodeURIComponent(`+91${item.vendor_mobile}`)}`
            ...
            <View style={{ margin: 10 }}>
                <MyButton btnname="Add New Vendor" background="#173161" fontsize={18} textcolor="#FFF" runFunc={() => navigation.navigate('AddVendor')} />
            </View>
            {/* )} */}
